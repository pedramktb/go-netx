...
func newAESPair(t *testing.T) (client net.Conn, server net.Conn) {
	...
	go func() { c, ec = netx.NewAESGCMConn(fc, key); done <- struct{}{} }()
	go func() { s, es = netx.NewAESGCMConn(fs, key); done <- struct{}{} }()
	<-done
	<-done
	if ec != nil {
...
func TestAESGCM_DecryptErrorWrongKey(t *testing.T) {
	...
	go func() { c, ec = netx.NewAESGCMConn(fc, keyA); done <- struct{}{} }()
	go func() { s, es = netx.NewAESGCMConn(fs, keyB); done <- struct{}{} }()
	<-done
	<-done
	if ec != nil {
		t.Fatalf("client: %v", ec)
